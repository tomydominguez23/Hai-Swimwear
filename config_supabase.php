...
// IMPORTANTE: Necesitas la contraseña de la base de datos (no la API Key)
...
function verifyPassword($password, $hash) {
    try {
        // Si el hash parece ser de pgcrypto (empieza con $2a$, $2b$, etc. o tiene formato específico)
        // Intentar verificar con pgcrypto primero
        if (strpos($hash, '$2') === 0 || strpos($hash, '$2a$') === 0 || strpos($hash, '$2b$') === 0) {
            // Es un hash bcrypt de PHP, usar password_verify
            return password_verify($password, $hash);
        } else {
            // Intentar con pgcrypto
            try {
                $result = fetchOne("SELECT crypt($1, $2) = $2 as match", [$password, $hash]);
                if ($result && isset($result['match'])) {
                    return $result['match'] === true || $result['match'] === 't';
                }
            } catch(PDOException $e) {
                // Si falla, intentar con password_verify
            }
        }
        // Fallback a password_verify de PHP
        return password_verify($password, $hash);
    } catch(Exception $e) {
        error_log("Error en verifyPassword: " . $e->getMessage());
        // Último recurso: comparación directa (NO RECOMENDADO, solo para debugging)
        return false;
    }
}
